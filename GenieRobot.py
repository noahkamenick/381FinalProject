...
def sys_info(incoming_msg):
    ...
    if len(info) == 0:
        ...
        response.markdown += "Device type: {}.\nSerial-number: {}.\nCPU Type:{}\n\nSoftware Version:{}\n".format(
            info["device-inventory"][0]["hw-description"],
            info["device-inventory"][0]["serial-number"],
            info["device-inventory"][4]["hw-description"],
            info["device-system-data"]["software-version"],
        )
    ...
    for intf in intf_list:
        ...
        try:
            response.markdown += "IP Address:{}\{}\n".format(
                intf["ietf-ip:ipv4"]["address"][0]["ip"], intf["ietf-ip:ipv4"]["address"][0]["netmask"]
            )
        except KeyError:
